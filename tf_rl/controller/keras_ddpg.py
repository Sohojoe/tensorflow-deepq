...
class KerasDDPG(object):
    ...
    def plot_critic_value_function(self, save=False, filename=None):
        ...
        dx, dy = 0.05, 0.05
        y, x = np.mgrid[slice(-1*np.pi, 1*np.pi+dy, dy), slice(-np.pi, np.pi+dx, dx)]
        ...
        for ax, action_multiplier in zip(axes.flat, np.linspace(-1.0, 1.0, num=21)):
            ...
            for i in range(len(q_0v_angle)):
                new_q[i/len(x), i%len(x[0])] = q_0v_angle[i]
    ...
    def plot_actor_policy(self, save=False, filename=None):
        ...
        states = np.zeros((len(y)*len(x), self.observation_size))
        ...
        for i in range(len(actions)):
            new_a[i/len(x), i%len(x[0])] = actions[i]
            new_a_t[i/len(x), i%len(x[0])] = actions_t[i]
    ...
    def action(self, observation, dt, ignore_exploration=False):
        ...
        if self.exploration_period > self.actions_executed_so_far and not ignore_exploration:
