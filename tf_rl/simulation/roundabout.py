...
class RoundaboutMap(object):
    def __init__(self, settings):
        self.settings = settings


class RoundaboutSim(BaseSimulation):
    def __init__(self, settings):
        ...
        self.hero = RoundaboutVehicle(Point2(*self.settings["hero_initial_position"]),
    ...
    def perform_action(self, action):
        ...
        self.hero.speed = action

    def spawn_object(self, obj_type):
        """Spawn object of a given type and add it to the objects array"""
        # Spawn objects according to a Poisson distribution
        # Prob of num of cars on a route in an episode
        ...
        self.objects.append(RoundaboutVehicle(position, speed, obj_type, self.settings))
    ...
    def resolve_collisions(self):
        ...
        for obj in to_remove:
            ...
            self.object_reward += self.settings["collision_reward"][obj.obj_type]
            self.spawn_object(obj.obj_type)

    def resolve_terminal_states(self):
        # Check if our car is in collision with other vehicle
        if self.resolve_collisions():
            # Mark as terminal state with very negative reward
            self.terminal_state = True
        else: # Check if cars have achieved their goal state and if so remove them and spawn a new vehicle
            achievement_distance_2 = self.settings["object_radius"] ** 2
            to_remove = []
            for obj in self.objects:
                if self.squared_distance(obj.goal_position, obj.position) < achievement_distance_2:
                    to_remove.append(obj)
            for obj in to_remove:
                self.objects.remove(obj)
                self.spawn_object(obj.obj_type)
